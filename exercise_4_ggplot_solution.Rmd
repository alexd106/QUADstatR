...
8. When exploring your data it is often useful to visualise the distribution of continuous variables. Take a look at [Section 5.4.2](https://intro2r.com/bestiary.html#histogram) and then create histograms for the variables; `DML`, `weight`, `eviscerate.weight` and `ovary.weight`. Again, it's up to you if you want to plot all 4 plots separately or in the same figure. Export your plot(s) as pdf file(s) to the `output` directory. One potential problem with histograms is that the distribution of data can look quite different depending on the number of 'breaks' or 'bins' used. The `geom_histogram` function does its best to create appropriate 'breaks' for your plots (it uses the [Sturges algorithm](https://en.wikipedia.org/wiki/Histogram) for those that want to know) but experiment with changing the number of breaks for the `DML` variable to see how the shape of the distribution changes (see [Section 5.4.2](https://intro2r.com/bestiary.html#histogram) of the book for further details of how to change the bins).   
